...
def sample_results():
    prestataire = Prestataire(
        ...
        services=["réparation fuite", "débouchage"],
        city="Paris",
        country="France",
        hourly_rate=55.0,
        rating=4.7,
    )
    return [SearchResult(prestataire=prestataire, similarity_score=0.85)]
...
def test_format_results(chat_svc, sample_results):
    ...
    assert len(parsed) == 1
    assert parsed[0]["name"] == "Jean Dupont Plomberie"
    assert parsed[0]["hourly_rate"] == 55.0
...
async def test_generate_response(chat_svc, sample_results):
    ...
    response = await chat_svc.generate_response(
        "Je cherche un plombier", sample_results
    )
